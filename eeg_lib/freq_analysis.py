...
def plot_stft_spectra(Sxx, f, ssvep_f0=None, figsize=(14,12), recursive_av=True, f_ssvep=[8.75, 10, 12, 15]):
    ...
    n_win = Sxx.shape[1]
    fig, axes = plt.subplots(n_win,3, figsize=figsize, )
    for i in range(n_win):
        Sxx_av = np.mean(Sxx[:, 0:i+1], axis=1)
        ax_psd = plot_spectrum(axes[i][0], f, Sxx[:, i], ssvep_f0=ssvep_f0, title=f'Window {i+1}')
        ax_zoom = plot_spectrum(axes[i][1], f, Sxx[:, i], ssvep_f0=ssvep_f0, title=f'Window {i+1}')
        
        if recursive_av:
            plot_spectrum(ax_psd, f, Sxx_av)
            plot_spectrum(ax_zoom, f, Sxx_av)
            
        ax_zoom.set_xlim(min(f_ssvep)*0.75, max(f_ssvep)*1.25) # zoom in on freq band of interest
...
def decode_ssvep(Sxx, f, target_freqs, convert_to_mag=True, verbose=False):
    ...
    for fsv in target_freqs:
        ...
        if f_idx != fsv and verbose:
            print(f"Warning: couldn't find exact match for SSVEP freq {fsv}. Using {f[f_idx]}")
        p.append(Sxx[f_idx])
