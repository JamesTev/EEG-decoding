...
def iir_bandpass(x, f0, f1, fs, order=4, ftype="cheby2"):
    b, a = iirfilter(
        order, [f0, f1], rs=60, btype="band", fs=fs, ftype=ftype, output="ba"
    )
    y = filtfilt(b, a, x)
    return y
...
def iir_bandpass_ssvep_tensor(X, f0, f1, fs, order=4, ftype="cheby2"):
    ...
    for trial in range(Nt):  # TODO parallelise or similar
        for f_idx in range(Nf):
            X_i = X[f_idx, :, :, trial]
            X_f[f_idx, :, :, trial] = np.array(
                [bp_filt(X_i[chan, :]) for chan in range(Nc)]
            )
    return X_f
...
def filterbank(eeg, fs, idx_fb):
    ...
    [N, Wn] = scipy.signal.cheb1ord(
        Wp, Ws, 3, 40
    )  # band pass filter StopBand=[Ws(1)~Ws(2)] PassBand=[Wp(1)~Wp(2)]
    ...
    if num_trials == 1:
        for ch_i in range(num_chans):
            ...
            y[ch_i, :] = scipy.signal.filtfilt(
                B, A, eeg[ch_i, :], padtype="odd", padlen=3 * (max(len(B), len(A)) - 1)
            )

    else:
        for trial_i in range(num_trials):
            for ch_i in range(num_chans):
                y[ch_i, :, trial_i] = scipy.signal.filtfilt(
                    B,
                    A,
                    eeg[ch_i, :, trial_i],
                    padtype="odd",
