...
class TRCA:
                ...
                for k in range(Nt):
                    for l in range(Nt):
                        if k != l:  # compare blocks (trials) l and k
                            ...
                            S[i, j] += np.dot(
                                (xi - np.mean(xi, axis=1)), (xj - np.mean(xj, axis=1)).T
                            )

        X_bar = X.reshape((Nc, Ns * Nt)) - np.tile(
            X.reshape((Nc, Ns * Nt)).mean(axis=1).reshape(Nc, 1), (1, Ns * Nt)
        )
    ...
    def compute_corr(self, X_test):
        ...
        return pearsonr(
            np.squeeze(w.T.dot(X_test)), np.squeeze(np.squeeze(w.T.dot(X_av)))
        )[0]
