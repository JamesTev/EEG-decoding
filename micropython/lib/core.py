...
def initialise(offline=False, spi_params=None, adc_params=None):
    ...
    stim_freqs = [7, 10, 12]
    cca_decoder = CCA(
        stim_freqs, cca_ref_freq
    )  # !! use DOWNSAMPLED_FREQ for harmonic reference if using downsampling on original signal
...
def read_and_decode(preprocessing=True):
    ...
    gc.collect()
    decoded_result = {
        freq: round(corr[0], 4) for freq, corr in cca_decoder.compute_corr(data).items()
    }
    gc.collect()
    ...
    if MODE != "log":
        while True:
            ...
            payload = pack_payload(
                [1, 2, 3], decoded_data, client_id=mqtt_client.client_id
            )
            gc.collect()
