...
class ScheduledFunc:
    ...
    def start(self, callback=None):
        callback = (
            callback or self.cb
        )  # use callback if supplied, else default class callback
        self.tim.init(
            freq=self.freq, callback=callback
        )  # !! freq kwarg was only added in mpy v1.16
...
class WsDataScheduler(ScheduledFunc):
    ...
    def cb(self, timer, *args):
        ...
        micropython.schedule(
            self.send_data, self
        )  # see https://docs.micropython.org/en/latest/library/micropython.html?highlight=schedule#micropython.schedule
