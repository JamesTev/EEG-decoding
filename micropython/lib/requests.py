...
class MicroWebCli:
    ...
    def GETRequest(
        url, queryParams=None, auth=None, connTimeoutSec=10, socks5Addr=None
    ):
        c = MicroWebCli(
        ...
        if r.IsLocationMoved():
            return MicroWebCli.GETRequest(
                r.LocationMovedURL(), queryParams, auth, connTimeoutSec, socks5Addr
            )
        return None
    ...
    def POSTRequest(url, formData={}, auth=None, connTimeoutSec=10, socks5Addr=None):
        ...
        r.Close()
        if r.IsLocationMoved():
            return MicroWebCli.POSTRequest(
                r.LocationMovedURL(), formData, auth, connTimeoutSec, socks5Addr
            )
        return None
    ...
    def JSONRequest(url, o=None, auth=None, connTimeoutSec=10, socks5Addr=None):
        ...
        r.Close()
        if r.IsLocationMoved():
            return MicroWebCli.JSONRequest(
                r.LocationMovedURL(), o, auth, connTimeoutSec, socks5Addr
            )
        return None
    ...
    def FileRequest(
        ...
        r.Close()
        if r.IsLocationMoved():
            return MicroWebCli.FileRequest(
                r.LocationMovedURL(),
                filepath,
                progressCallback,
                auth,
    ...
    def __init__(
        self, url="", method="GET", auth=None, connTimeoutSec=10, socks5Addr=None
    ):
        self.URL = url
        self.Method = method
        self.Auth = auth
    ...
    def OpenRequest(self, data=None, contentType=None, contentLength=None):
        ...
        try:
            ...
            cli = socket.socket(socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_TCP)
        ...
        if self.Socks5Addr:
            ...
            try:
                ...
                if b is None or len(b) < 2 or b[0] != 0x05 or b[1] != 0x00:
                    err = (
                        "%s:%s doesn't supports MicroWebCli SOCKS5 client protocol"
                        % self.Socks5Addr
                    )
                else:
                    h = self.Host.encode()
            ...
            for param in formData:
                if param != "":
                    if data != "":
                        data += "&"
                    data += (
                        MicroWebCli._quote(param)
                        + "="
                        + MicroWebCli._quote(formData[param])
                    )
        self.OpenRequest(data=data, contentType="application/x-www-form-urlencoded")
    ...
    class _response:
        ...
        def _parseFirstLine(self):
            self._httpVer, code, self._msg = (
                self._socket.readline().decode().strip().split(" ", 2)
            )
            self._code = int(code)
